/**
 * Static reference data and the shared colour palette.
 *
 * This file used to also carry a mock backend: a seeded random-walk price
 * generator, a fabricated forecast generator, hardcoded P/E ratios and market
 * caps for nine tickers, client-side SMA/EMA/RSI/MACD/Bollinger implementations,
 * and an `optimizePortfolio` that returned normalised random numbers. All of it
 * was scaffolding from before the API existed, none of it was still imported,
 * and it was actively dangerous to keep:
 *
 *   * `optimizePortfolio` was also the name of the REAL optimizer in utils/api.js,
 *     which is what both portfolio tabs import. One wrong auto-import and a
 *     holdings table would have shown random weights, with nothing on screen to
 *     say so.
 *   * The indicator functions were a second, silently divergent implementation of
 *     maths the server already computes and sends. Two RSIs in one product is one
 *     RSI too many.
 *   * `FUNDAMENTALS` was invented financial data presented in the same shapes as
 *     the real thing.
 *
 * What remains is what is actually used: the index symbol, the default and legacy
 * watchlists, the S&P 500 constituent snapshot the picker falls back to when the
 * API is unreachable, and the palette.
 */
